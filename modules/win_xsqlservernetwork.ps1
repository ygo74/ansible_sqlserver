...
#ATTRIBUTE:ProtocolName;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Tcp
$ProtocolName = Get-AnsibleParam -obj $params -name ProtocolName -failifempty $True -resultobj $result
#ATTRIBUTE:IsEnabled;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$IsEnabled = Get-AnsibleParam -obj $params -name IsEnabled -failifempty $False -resultobj $result
...
#ATTRIBUTE:RestartService;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$RestartService = Get-AnsibleParam -obj $params -name RestartService -failifempty $False -resultobj $result
#ATTRIBUTE:RestartTimeout;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$RestartTimeout = Get-AnsibleParam -obj $params -name RestartTimeout -failifempty $False -resultobj $result
...
#ATTRIBUTE:TcpDynamicPorts;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$TcpDynamicPorts = Get-AnsibleParam -obj $params -name TcpDynamicPorts -failifempty $False -resultobj $result
#ATTRIBUTE:TcpPort;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$TcpPort = Get-AnsibleParam -obj $params -name TcpPort -failifempty $False -resultobj $result
...
If ($ProtocolName)
{
    If (('Tcp') -contains $ProtocolName ) {
    }
    Else
    {
        Fail-Json $result "Option ProtocolName has invalid value $ProtocolName. Valid values are 'Tcp'"
    }
}
...
If ($TcpDynamicPorts)
{
    If (('','0') -contains $TcpDynamicPorts ) {
    }
    Else
    {
        Fail-Json $result "Option TcpDynamicPorts has invalid value $TcpDynamicPorts. Valid values are '','0'"
    }
}
...
$DscResourceName = "xSQLServerNetwork"
