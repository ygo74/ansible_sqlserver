...
#ATTRIBUTE:RecoveryModel;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:BulkLogged,Full,Simple
$RecoveryModel = Get-AnsibleParam -obj $params -name RecoveryModel -failifempty $True -resultobj $result
...
If ($RecoveryModel)
{
    If (('BulkLogged','Full','Simple') -contains $RecoveryModel ) {
    }
    Else
    {
        Fail-Json $result "Option RecoveryModel has invalid value $RecoveryModel. Valid values are 'BulkLogged','Full','Simple'"
    }
}
...
$DscResourceName = "xSQLServerDatabaseRecoveryModel"
